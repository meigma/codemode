	...
	tests := []struct {
		...
		},
		{
			...
			cues: []string{
				...
				"math.*",
				"directly available without import",
				"import, while, f-strings, filter, and map are unavailable",
				"load is disabled",
				"print is discarded",
			},
...
func requireFieldShapeItemSchema(t *testing.T, items any) {
	...
	require.Len(t, required, 3, "FieldShape must require exactly name, type, and required")
	properties := requireJSONObject(t, item["properties"])
	require.Len(t, properties, 3, "FieldShape must stay a flat name/type/required object")
	requireNonNullJSONType(t, requireJSONObject(t, properties["name"]), "string")
