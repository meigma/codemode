...
## 2026-08-24 23:03 — Increment 4 implementation complete
Parallel Programmer slices completed `internal/worker`: marker-only same-binary child serving, full-engine execution, native-call forwarding, classified final results, a fresh-process parent `Runner`, immutable manifest and limits, a live-child semaphore, five-second build probe, context-aware supervision, deterministic pipe closure, kill, exactly-once reap, and permit release. The public surface remains the frozen `ErrProtocol`, `IsWorker`, `Serve`, `Dispatch`, `Limits`, `Runner`, `NewRunner`, `Probe`, and `Execute` API.
...
## 2026-08-25 09:19 — Increment 5 implementation complete
Completed the atomic worker-only public cutover on `feat/worker-only-execution`. Root `Builder.Build` now derives the filtered process-neutral manifest, constructs and probes one immutable `worker.Runner`, and retains it on `Server`; every valid `Server.Execute` delegates to a fresh re-executed worker. Added the public `IsWorker` and `ServeWorkerAndExit` facades, mandatory root and MCP `TestMain` entries, asynchronous cancellation-aware parent authorization and handler dispatch, parent output depth/byte validation, and clean removal of `MaxResultBytes`, in-process elapsed cancellation, `ConvertFinal`, and the temporary engine final-byte wrapper.

Added same-binary cutover regressions for the worker probe and miswired diagnostic, parent-PID native forwarding, exact `int64`/integral-`float64`/negative-zero preservation, the two exact issue #12 monolithic built-ins, cancellation and reap, queued cancellation, abort preservation, concurrency caps, repeated kill/reap, worker-frame overflow, and public limit validation. Updated package Godoc, README, SECURITY, tutorial, API/MCP reference, documentation home, and security model with first-statement `main`/`TestMain` wiring, five-second probe semantics, complete execution-budget coverage, the marker-only child environment, parent-code cancellation limits, and the absence of operating-system tenant quotas.

Final verification passes: scoped `golangci-lint` reports zero issues; `mise exec -- go test ./... -count=1 -timeout=120s`; both required internal-worker and root `-race` gates; `mise exec -- moon run docs:build` under strict MkDocs validation; `GOOS=windows GOARCH=amd64 go build ./...`; and `git diff --check`.
