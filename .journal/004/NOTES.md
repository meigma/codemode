...
## 2026-08-24 10:38 — Increment 6 merged and Increment 7 implemented
Merge: PR #21 passed CI, GitHub Pages, and Kusari Inspector, then squash-merged to `master` as `7c77a80`.
Request: Continue with Increment 7 after merging Increment 6.
Implementation: Created `docs/error-boundary-contract` from the fetched merged `origin/master`. The security explanation and public references now distinguish detailed trusted-side causes, direct `authz/rego.Authorize` errors, root `Server.Execute` sentinel projection, fixed MCP error texts, and pre-resolution SDK input-schema errors. Root execution retains only the documented `context.Canceled` result and `ErrResourceLimit` wrapping of `context.DeadlineExceeded`; policy, handler, execution, and internal causes are discarded. MCP resolver/custom-service detail and recovered panic values remain redacted. The root and MCP projection comments now state those boundaries without changing behavior.
Recovery and policy inference: The MCP reference directs clients to search and describe exact names, compare calls with published signatures and field shapes, repair the zero-argument `main` form, reduce source or result complexity after resource exhaustion, and contact the host for unexpected authorization outcomes without echoing failed input. One allowed or denied call is explicitly insufficient to infer whether policy is default-open, default-deny, complete, or incomplete.
Verification: Focused direct Rego, root projection, MCP projection, panic-sanitization, and secure-loop tests passed. The campaign canary witness produced 17 MCP responses with zero canary matches while all five fixture positive controls contained the canary. Nine response-only M08 error payloads had zero matches across 13 source, argument, credential, policy, handler, panic, and internal-detail terms. `mise exec -- moon run root:check` completed all eight tasks. LSP diagnostics reported no issues in the two changed Go files, and `git diff --check` passed. Temporary campaign modfiles were removed, and the implementation branch tracks no `.journal` files.
Result: Commit `c124ecd` pushed to `docs/error-boundary-contract`; PR #22 opened at https://github.com/meigma/codemode/pull/22. GitHub CI, Pages, and inspection checks were in progress when the PR was checked.

## 2026-08-24 10:39 — Increment 7 pull request checks passed
Verification update: PR #22 passed CI, GitHub Pages, and Kusari Inspector.

## 2026-08-24 11:26 — Increment 7 merged and Increment 8 completed
Merge: PR #22 passed its checks and squash-merged to `master` as `ccac0332a64c52530ebf0102849f73eecf867a12`.
Request: Continue with Increment 8, which is the independent disposable-harness repair in `FINDINGS_REMEDIATION_PLAN.md`.
Implementation: Repaired `/Users/josh/code/meigma/codemode-functional-004` without modifying the CodeMode repository. C08 now has discriminating copied-slice and copied-limit controls. C09 combines concurrent unique-token checks with writer/read-only stale-global and valid negative-control programs. Typed catalog values retain declared JSON field order. Resolver, authorizer, and handler observations share one event sequence. E13 uses sequential fresh-budget calls and valid state probes; E14/E16 isolate source, native-call, depth, result, step, execution-time, and request-deadline budgets; E17 derives its ledger from retained classified evidence and exact paths. Rego probes now use a genuinely non-ground reference and no longer claim reachable multiple results. MCP observations no longer duplicate resolver events or synthesize an empty-subject row, retain the corrected M07 over-limit value, and separate emitted arguments from host-dispatched authorization and handler records.
OMP harness: Promoted the accepted U03, U04, and reusable U06 prompt forms. JSON mode, one fresh server, tool-read/tool-write/non-MCP accounting, resolver reconciliation, and emitted-versus-dispatched argument retention are explicit in `commands.md`. Superseded U03/U04/U06 records remain preserved as negative evidence.
EOF correction: A complete raw stdio session exposed an additional SDK v1.7 detail: `Server.Run` may return `server is closing: EOF` without wrapping `io.EOF`. The fixture host recognizes that exact SDK form as clean, also treats a nil stdio session result as clean, emits `clean stdin EOF`, and still exits nonzero for an unknown fixture.
Verification: The repaired oracle audit passed for C08, C09, D06, A01, E07, E13, E14, E16, E17, R02, R03, M05, and M07. A race-built `--group all` run emitted all 211 observations without a race report. The focused owned-package race command passed. The clean-install script resolved `github.com/meigma/codemode@master` from the module cache, extracted the published tutorial, ran it, and matched search, describe, and execute output. A real initialize plus `tools/list` stdio session returned both responses, then closed on EOF with exit 0, empty stderr, and clean host evidence; an unknown fixture still exited 1. Fresh one-shot OMP JSON runs completed the promoted U03, U04, U06 allow, undefined, non-Boolean, and builtin-failure prompts using only the three CodeMode MCP tools. LSP diagnostics reported no issues in the harness command, fixture, observation, or probe packages.
Result: Increment 8 is complete in the retained disposable harness. The harness is intentionally outside Git and CodeMode; no product branch or pull request exists for this increment. Generated binaries and temporary verification workspaces were removed.

## 2026-08-24 12:19 — Increment 9 completed
Request: Merge the approved Increment 8 work and continue with the next increment.
Merge disposition: Increment 8 is the plan's disposable non-Git harness increment, so it has no branch or pull request to merge. Its retained source was already final. The CodeMode checkout has no uncommitted compensating change.
Implementation: Executed Increment 9 (`UP-01`) by filing [can1357/oh-my-pi issue #9653](https://github.com/can1357/oh-my-pi/issues/9653), “MCP silently rewrites model-emitted argument keys and drops unknown fields.”
Evidence: The report includes exact sanitized pairs from the retained JSON transcripts: four U03 `code` → `source` rewrites, four U04 `program` → `source` rewrites including one call where non-empty `subject`, `role`, and `key` fields disappeared, and two U06 `code` → `source` rewrites. It identifies `tool_execution_start.args.content` as model-emitted and `tool_execution_end.result.details.xdev.args` as dispatched. Direct official SDK controls show that missing `source` and an extra `subject` are schema-rejected with resolver, authorizer, and handler counts all zero when the original object reaches the server.
Upstream request: Either dispatch the emitted MCP object exactly and surface schema validation, or explicitly disclose normalization to the model, user, and transcript. The issue rejects CodeMode aliases, relaxed `additionalProperties`, and server-side rewrite heuristics.
Verification: Searched open and closed upstream issues before filing; no matching key-rewrite report existed. The published issue is open under `jmgilman`, contains both renamed-key and dropped-field reproductions, and records macOS, OMP `17.3.4`, Bun `1.3.14`, and the Codex provider. No CodeMode source, configuration, documentation, dependency, or test file changed.
Result: Increment 9 is complete. All nine findings-remediation increments are now executed.

## 2026-08-24 12:26 — Close
Outcome: Session goal met. The 59-case external functional campaign, evidence QA, complete findings-remediation plan, seven CodeMode product/documentation increments, disposable harness repair, and upstream OMP report are complete.
Merged work: [PR #16](https://github.com/meigma/codemode/pull/16), [PR #17](https://github.com/meigma/codemode/pull/17), [PR #18](https://github.com/meigma/codemode/pull/18), [PR #19](https://github.com/meigma/codemode/pull/19), [PR #20](https://github.com/meigma/codemode/pull/20), [PR #21](https://github.com/meigma/codemode/pull/21), and [PR #22](https://github.com/meigma/codemode/pull/22) are squash-merged. Local `master` is fast-forwarded to `ccac0332a64c52530ebf0102849f73eecf867a12`, and all seven implementation worktrees and local branches were removed.
Handoff: The repaired source harness and retained evidence remain at `/Users/josh/code/meigma/codemode-functional-004`; no generated binaries or temporary modules remain. [OMP issue #9653](https://github.com/can1357/oh-my-pi/issues/9653) is the only active external thread. CodeMode must retain its strict schemas while that client behavior is resolved.
Artifacts: `SUMMARY.md` is the cold-start postmortem, `FUNCTIONAL_TEST_PLAN.md` records the campaign, `FINDINGS_REMEDIATION_PLAN.md` records every disposition, and `.journal/TECH_NOTES.md` now carries the durable product and transcript facts.
